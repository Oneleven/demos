...
signUpForm.addEventListener('submit', (e) => {
  ...
  if(password.length === 0){
     $('#error_signUp_password').text("密码不能为空！").show().fadeOut(1000)
    }else if(password.length <6){
     $('#error_signUp_password').text("密码不能少于6位！").show().fadeOut(1000)
     return 
    }
  // 新建 AVUser 对象实例
  ...
  user.signUp().then(function (loginedUser) {
    ...
    console.log(error)
    if(error.code === 203){
     $('#error_signUp_email').text("此电子邮箱已被占用！").show().fadeOut(1000) 
    }else if(error.code === 202){
     $('#error_signUp_username').text("此用户名已被注册！").show().fadeOut(1000) 
    }
  })
})
...
logInForm.onsubmit = (e) => {
  ...
  AV.User.logIn(username, password).then(function (loginedUser) {
    ...
    console.log(error)
    if (error.code === 210) {
      $('#error_logIn_password').text("密码错误！").show().fadeOut(1000)
    } else if (error.code === 219) {
      $('#error_logIn_password').text("登录失败次数超过限制，请稍候再试!").show().fadeOut(1000)
    } else if (error.code === 211) {
      $('#error_logIn_username').text("用户不存在！").show().fadeOut(1000)
    }
    console.log('ok')
  });
